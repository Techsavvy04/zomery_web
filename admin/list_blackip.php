<?php
$title_backip='Danh sách blackip';
    ...
    <div class="content">
        <div class="block block-rounded">
            <div class="block-header block-header-default">
                <h3 class="block-title"> Danh sách blacklist </h3>
            ...
            <div class="block-content block-content-full">
                ...
                <table class="text-center" id="listTable" border="1" data-toggle="table" data-search="true"
                    ...
                    <tbody>
                    <?php
$showblackip = "SELECT id, blackip FROM setting";
$result_blackip = $thanhdieudb->query($showblackip);

if ($result_blackip === false) {
    echo "Error: " . $thanhdieudb->error;
} else {
    if ($result_blackip->num_rows > 0) {
        $count = 1;
        while ($row = $result_blackip->fetch_assoc()) {
            $ips = explode('|', $row['blackip']);
            foreach ($ips as $ip) {
?>
                <tr>
                    <td><?= $count++; ?></td>
                    ...
                    <td>
                        <div class="btn-group">
                            <a class="btn btn-xs btn-danger" data-toggle="tooltip">Bị cấm truy cập</a>
                        </div>
                    </td>
                </tr>
<?php
